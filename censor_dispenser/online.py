...
negative_words = ["concerned", "behind", "danger", "dangerous", "alarming", "alarmed", "out of control", "help",
                  "unhappy", "bad", "upset", "awful", "broken", "damage", "damaging", "dismal", "distressing",
...
def encryptor_one_word(word, email):
    ...
    while i < len(email):
        ...
        if (
                to_be_encrypted == word.lower() or to_be_encrypted == word.upper() or to_be_encrypted == word.title() or to_be_encrypted == word):
...
def encryptor_several_words(words, email):
    ...
    while i < len(email):
        for j in words:
            ...
            to_be_encrypted = email[i:i + length]
            if (
                    to_be_encrypted == j.title() or to_be_encrypted == j.upper() or to_be_encrypted == j.lower() or to_be_encrypted == j):
...
def encryptor_bad_language(neg_words, prop_words, email):
    ...
    while i < len(email):
        for trigger in neg_words:
            ...
            if (
                    trigger == lol and count <= 2 or trigger.upper() == lol and count <= 2 or trigger.title() == lol and count <= 2 or trigger.lower() == lol and count <= 2):
                count += 1
                continue
