...
# emails are contained in and the .read() method is allowing us to save their contexts to the following variables:
...
def censor_three(input_text, censored_list, forbidden_words):
    ...
    for i in range(0, len(input_text_words)):
        if input_text_words[i] in censored_list:
        ...
        count = 0
        for i in range(0, len(input_text_words)):
            if input_text_words[i] in forbidden_words:
                count += 1
                if count > 2:
                    word_clean = input_text_words[i]
